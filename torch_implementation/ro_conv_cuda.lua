-- gpu version of the same ro_conv layer
require 'paths'
require 'cunn'
require 'monteCarloProbRotation_cuda' -- the module has a method mcpr
require 'math'
...
function Roconv:__init(nInputAng,nOutputAng,nInputFilter,nOutputFilter,kH,kW,dH,dW,circular,rp,loss)
   ...
   self.W = torch.CudaTensor(nOutputFilter,nInputFilter,nInputAng,kH,kW)
   self.bias = torch.CudaTensor(nOutputFilter)
   self.gradW = torch.zeros(self.W:size()):cuda(); self.gradWeight = torch.CudaTensor() -- for compatibility wit 3d conv
   self.gradB = torch.zeros(self.bias:size()):cuda(); self.gradBias = torch.CudaTensor() -- for compatibility wit 3d conv
   self:reset()
   -- other parameters
   opt.verbose = false
   opt.debug = false
...
function Roconv:__tostring__()
   ...
   return torch.type(self) ..
      string.format('(%d->%d, %dx%d rf, %d->%d ang, %f rp)', self.nInputFilter, self.nOutputFilter,
		    self.W:size(4), self.W:size(5), self.nInputAng, self.nOutputAng, self.rp)
end
...
function Roconv:updateOutput(input) -- reference https://github.com/torch/nn/blob/master/doc/convolution.md#nn.VolumetricConvolution
   ...
   print(output:size())
   output = output:transpose(3,5):transpose(3,4)
   ...
   for i=2,self.nOutputAng do 
	 ...
	 old_weight = self.weight
      ...
      local oTemp = input.nn.VolumetricConvolution_updateOutput(self, input):transpose(3,5):transpose(3,4)
      output = torch.cat(output:double(),oTemp:double(),rotDim):cuda()
   end
   self.output = output
   if opt.verbose then
      print('output is ')
...
function Roconv:accGradParameters(input, gradOutput, scale)
   ...
   self.gradWeight = torch.zeros(self.weight:size()):cuda(); 
   self.gradBias = torch.zeros(self.bias:size()):cuda()
   ...
   if input:dim()==4 then
	 ...
	 gW = torch.cat(gW:double(),self.gradWeight:double(),6):cuda(); self.gradWeight:zero(); self.gradBias:zero();
      end
      -- weight: nOutputFilter x nInputFilter x nInputAng x kH x kW
