% other datasets: createNewDataset.m
only_ten = true;
% positive are ones, negtive are rotated 90 degree ones plus other 
MNIST_loaded = 0;
load_MNIST_data;
pos = train_data(:,:,:,train_label==1); % 6742 positive examples
tmp = zeros(size(pos)); % another half of pos \--|
for i=1:size(pos,4) tmp(:,:,:,i) = imrotate(pos(:,:,:,i),90); end
pos = cat(4,pos,tmp);
...
for i=1:size(neg,4)
    if only_ten
        curr_data = pos(:,:,:,i);
        % should adjust brightness by averaging the centered part
        rot = imrotate(curr_data,90); 
        diff = curr_data - rot;
        diff(diff<0) = 0;
        neg(:,:,:,i) = rot+diff;        
    else
